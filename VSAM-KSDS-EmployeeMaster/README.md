...
## Why This Project Is a Differentiator

- Shows **both** VSAM access modes in one coherent project (SEQUENTIAL
  and RANDOM), not just a toy "hello world" read.
- Realistic **file-status checking** and error handling (`INVALID KEY`,
  duplicate-key rejection, not-found handling).
- A genuine **control-break report** (department subtotal + grand
  total) — a very common real-world COBOL pattern interviewers ask about.
- Complete, runnable **JCL** for DEFINE, COMPILE, and three execution
  steps — showing you understand the full batch lifecycle, not just
  the COBOL source.

See `docs/INTERVIEW-NOTES.md` for how to talk about this project in interviews.
